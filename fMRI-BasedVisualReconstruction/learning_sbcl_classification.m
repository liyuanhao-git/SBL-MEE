function [ww,IdxEff]=learning_sbcl_classification(tt,X,nlearn)
...
for ilearn=1:nlearn
   ...
   % W-step
   wwEff=w_mcc_adam(wwEff,tt,aaEff,XEff,hh);

   % compute the inverse hessian matrix
   pp=1./(1+exp(-XEff*wwEff));
   ExpW=exp(-0.5*(tt-pp).^2/hh);
   bb=ExpW.*(((tt-pp).^2-1).*(pp.^2).*((1-pp).^2)+(1-2*pp).*(tt-pp).*(pp).*(1-pp));
...
function wFinal=w_mcc_adam(wInt,tt,aa,X,hh)
...
for ilearn=1:nlearn
    ...
    Grad=(sum(-X.*repmat(ExpW.*(tt-pp).*pp.*(1-pp),1,size(X,2)),1))'+aa.*wTemp;
